(function () {
  ...
  consoleWindowWithoutHeader.style.width = "600px";
  ...
  const tabs = ["Console", "General", "Settings"];
  const tabButtons = {};
  ...
  tabGeneral.id = "tabGeneral";
  tabGeneral.style.padding = "16px";
  const generalLinksContainer = document.createElement("div");
  generalLinksContainer.style.display = "flex";
  generalLinksContainer.style.flexWrap = "wrap";
  generalLinksContainer.style.gap = "8px";
  tabGeneral.appendChild(generalLinksContainer);
  ...
  settingCheckboxLabel.appendChild(document.createTextNode(" Show Execution Time"));
  ...
  tabContentContainer.appendChild(tabGeneral);
  ...
  function switchTab(tabName) {
    ...
    tabGeneral.style.display = tabName === "general" ? "block" : "none";
  ...
  function evaluate(expression) {
    ...
    function parseIdentifier() {
      ...
      if (start === index) throw new Error(`Expected identifier at position ${index}`);
      return expression.slice(start, index);
    }
    const helpMessages = {
      round: "round(x): Rounds x to the nearest integer.",
      roundToDecimalPlace: "roundToDecimalPlace(x, d): Rounds x to d decimal places.",
    ...
    function parseFactor() {
      ...
      if (index < expression.length && /[a-zA-Z_]/.test(expression[index])) {
        ...
        if (expression[index] === "(") {
          ...
          if (expression[index] !== ")") throw new Error(`Expected ')' at position ${index}`);
          index++;
    ...
    skipWhitespace();
    if (index < expression.length) throw new Error("Unexpected character at position " + index);
    return result;
  }

  // --- Draggable and Resizable Functionality ---
  let isDragging = false, dragOffsetX = 0, dragOffsetY = 0;
  let isResizing = false, resizeStartWidth = 0, resizeStartHeight = 0, resizeStartX = 0, resizeStartY = 0;
  ...
  const dragMouseUp = () => { isDragging = false; };
  ...
  const resizeMouseMove = (e) => {
    if (isResizing) {
      ...
      tabContentContainer.style.height = (newHeight - headerHeight - tabBarHeight - 16) + "px";
    }
  };
  const resizeMouseUp = () => { isResizing = false; };
  ...
  document.addEventListener("keydown", (e) => {
    if (document.activeElement !== consoleInput && e.key.toLowerCase() === "h") {
      consoleWindow.style.display = consoleWindow.style.display === "none" ? "block" : "none";
      e.preventDefault();
    }
  });
  ...
  // --- Load Links for the General Tab from GitHub ---
  async function loadGeneralLinks() {
    try {
      const linksResponse = await fetch("https://raw.githubusercontent.com/proplayer919/classtools/main/links.json");
      if (!linksResponse.ok) throw new Error("Failed to load links.json");
      const linksData = await linksResponse.json();
      for (const entry of linksData) {
        const urlResponse = await fetch(`https://raw.githubusercontent.com/proplayer919/classtools/main/${entry.link}`);
        if (!urlResponse.ok) throw new Error(`Failed to load ${entry.link}`);
        const url = (await urlResponse.text()).trim();
        const btn = document.createElement("button");
        ...
        generalLinksContainer.appendChild(btn);
      }
    } catch (error) {
      console.error(error);
      const errorMsg = document.createElement("div");
      errorMsg.innerText = "Error loading links.";
      generalLinksContainer.appendChild(errorMsg);
    }
  }
  loadGeneralLinks();
  ...
  settingCheckbox.addEventListener("change", function () {
    ...
    consolePrint("Flag 'flag.showExecutionTime' " + (showExecutionTime ? "enabled" : "disabled"));
  });
  // When the title input changes, update the header title.
  settingInput.addEventListener("input", function () {
    title.innerText = this.value.trim() !== "" ? this.value + " - Made by proplayer919" : "ClassHub - Made by proplayer919";
  });
